...
- [ ] **Remove vestigial `global.security.lanCIDRs`** — unused since the listener-split refactor; still declared in `base-chart/values.yaml:49` and `apps/generic/values.schema.json:84`, and the schema description ("LAN traffic is gated by source IP") is now factually wrong.
